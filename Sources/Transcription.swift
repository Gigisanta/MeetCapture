...
final class WhisperModelManager: @unchecked Sendable {
  ...
  static let domainPrompt = """
    ...
    Participantes: (los nombres de los asistentes). \
    Empresas y proyectos: (los nombres propios que se repiten). \
  ...
  var vadModelPathAccessor: String? {
    ...
    let silero = candidates.filter { $0.hasPrefix("ggml-silero") && $0.hasSuffix(".bin") }.sorted()
      .last
  ...
  private func selectBestModel(for preferred: WhisperModelSize) -> WhisperModelSize {
    ...
    func workingSetMB(_ m: WhisperModelSize) -> UInt64 {
      UInt64(m.estimatedMemoryGB * 0.6 * 1024.0)
    }
  ...
  private func periodicMemoryCheck() {
    ...
    if availableMB < 300 {
      ...
      if smaller != current {
        logger.warning(
          "Critically low memory (\(availableMB)MB) — downgrading \(current.rawValue)→\(smaller.rawValue)"
        )
  ...
  private func availableMemoryMB() -> UInt64 {
    ...
    // Use sysctl to get page size (avoids Swift 6 warning on vm_page_size global var)
    var pageSize: UInt64 = 16384
    var pagesz = vm_size_t(0)
    var psize = MemoryLayout<vm_size_t>.size
    sysctlbyname("vm.pagesize", &pagesz, &psize, nil, 0)
    if pagesz > 0 { pageSize = UInt64(pagesz) }
