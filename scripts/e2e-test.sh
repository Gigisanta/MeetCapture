...
if grep -q "6.2.0" "$ROOT/Resources/Info.plist"; then
    green "  ✓ Version 6.2.0 in Info.plist"
    PASS=$((PASS+1))
else
...
if grep -q "6.2.0" "$ROOT/Sources/SettingsView.swift"; then
    green "  ✓ Version 6.2.0 in SettingsView"
    PASS=$((PASS+1))
else
...
if grep -q "6.2.0" "$ROOT/Sources/PopoverContent.swift"; then
    green "  ✓ Version 6.2.0 in PopoverContent"
    PASS=$((PASS+1))
else
...
handoff = {
    ...
    'metadata': {'transcript_path': '/tmp/test.txt', 'app_version': '6.2.0'}
}
